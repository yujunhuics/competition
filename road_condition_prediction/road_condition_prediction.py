...
def gen_feats(path, mode='is_train'):
    ...
    if mode == 'is_train':
        ...
        df['label'] -= 1
        df['current_slice_id'] = df[0].apply(lambda x: int(x.split(' ')[2]))   #当前时间片id
        df['future_slice_id'] = df[0].apply(lambda x: int(x.split(' ')[3]))    #未来时间片id
    else:
        df['label'] = -1
...
def lgb_train(train_: pd.DataFrame, test_: pd.DataFrame, use_train_feats: list, id_col: str, label: str,
    ...
    params = {
        # 'learning_rate': 0.05,#0.05缩小了十倍
        'learning_rate': 0.05,#目前最大0729，0.616
        # 'learning_rate': 0.05,#目前最大0729，0.6116918888477663
        'boosting_type': 'gbdt',
        'objective': 'multiclass',
        'metric': 'None',  #原始
        'num_leaves': 31,
        'num_class': n_class,
        'feature_fraction': 0.8, #原始0.8
        'bagging_fraction': 0.8,
        'bagging_freq': 5,
        'seed': 1,
        'bagging_seed': 1,
        'feature_fraction_seed': 7,   #原始是7
        'min_data_in_leaf': 20,
        'nthread': -1,
        'verbose': -1
    }
...
if __name__ == "__main__":
    train_path = 'D:/dataset/traffic/20190727.txt'
    ...
    attr = pd.read_csv('D:/dataset/attr.txt', sep='\t',
                       names=['link', 'length', 'direction', 'path_class', 'speed_class', 'LaneNum', 'speed_limit',
                              'level', 'width'], header=None)
    '''
    names=[完整路段拓扑切分后的小段id标识，长度，方向，道路等级，限速等级，车道数，限速，功能等级，宽度]
    '''

    train = pd.read_csv('is_train_20190727.txt')
    ...
    sub.to_csv('public_baseline27.csv', index=False, encoding='utf8')